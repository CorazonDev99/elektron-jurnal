...
class FSOController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('lisok.fso.index', compact('userId', 'roleId'));
    }
    ...
    public function deleteRecord(Request $request)
    {
        ...
        try {
            ...
            \DB::table('fso')->whereIn('id', $ids)->delete();
    ...
    public function getData(Request $request)
    {
        ...
        $d->selectRaw("'' as `empty`, fso.id,fso.user_id, fso.role_id, DATE_FORMAT(fso.created_at, '%d-%m-%Y %H:%i') AS created_at,
                DATE_FORMAT(fso.updated_at, '%d-%m-%Y %H:%i') AS updated_at, fso.equipment, fso.problem, fso.solution,
                fso.responsible_person, DATE_FORMAT(fso.deadline, '%d-%m-%Y') AS deadline, fso.acknowledgment, fso.resolved, fso.reason, fso.employee_name");
        if ($request->filled('fio')) {
            $d->where('fso.employee_name', 'like', '%' . $request->fio . '%');
        }
        if ($request->filled('created_at')) {
            $d->whereDate('fso.created_at', '=', $request->created_at);
        }
        if ($request->filled('equipment')) {
            $d->where('fso.equipment', 'like', '%' . $request->equipment . '%');
        }
        if ($request->filled('problem')) {
            $d->where('fso.problem', 'like', '%' . $request->problem . '%');
        }
        if ($request->filled('solution')) {
            $d->where('fso.solution', 'like', '%' . $request->solution . '%');
        }
        if ($request->filled('responsible_person')) {
            $d->where('fso.responsible_person', 'like', '%' . $request->responsible_person . '%');
        }
        if ($request->filled('deadline_from') && $request->filled('deadline_until')) {
            $d->whereBetween('fso.deadline', [$request->deadline_from, $request->deadline_until])
                ->orWhereBetween('fso.deadline', [$request->deadline_from, $request->deadline_until]);
        }

        elseif ($request->filled('deadline_from')) {
            $d->where('fso.deadline', '>=', $request->deadline_from)
                ->orWhere('fso.deadline', '>=', $request->deadline_until);
        }
        if ($request->filled('resolved')) {
            $d->where('fso.resolved', 'like', '%' . $request->resolved . '%');
        }
        if ($request->filled('acknowledgment')) {
            $d->where('fso.acknowledgment', $request->acknowledgment);
        }
    ...
    public function updateRecord(Request $request)
    {
        ...
        try {
            ...
            \DB::table('fso')->whereIn('id', $ids)->update([
                'acknowledgment' => 1,
            ]);
    ...
    public function editRecord(Request $request)
    {
        ...
        $recordExists = \DB::table('fso')->where('id', $recordId)->exists();

        if (!$recordExists) {
            return response()->json(['message' => 'Запись с указанным ID не найдена.'], 404);
        }

        \DB::table('fso')->where('id', $recordId)->update($data);
        return response()->json(['status' => true]);
