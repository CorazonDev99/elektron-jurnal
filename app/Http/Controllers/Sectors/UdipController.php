...
class UdipController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->filled('created_at')) {
            $d->whereDate('udip.created_at', '=', $request->created_at);
        }
        if ($request->filled('equipment')) {
            $d->where('udip.equipment', 'like', '%' . $request->equipment . '%');
        }
        if ($request->filled('problem')) {
            $d->where('udip.problem', 'like', '%' . $request->problem . '%');
        }
        if ($request->filled('solution')) {
            $d->where('udip.solution', 'like', '%' . $request->solution . '%');
        }
        if ($request->filled('responsible_person')) {
            $d->where('udip.responsible_person', 'like', '%' . $request->responsible_person . '%');
        }
        if ($request->filled('deadline_from') && $request->filled('deadline_until')) {
            $d->whereBetween('udip.deadline', [$request->deadline_from, $request->deadline_until])
                ->orWhereBetween('udip.deadline', [$request->deadline_from, $request->deadline_until]);
        }

        elseif ($request->filled('deadline_from')) {
            $d->where('udip.deadline', '>=', $request->deadline_from)
                ->orWhere('udip.deadline', '>=', $request->deadline_until);
        }
    ...
    public function updateRecord(Request $request)
    {
        ...
        try {
            ...
            \DB::table('udip')->whereIn('id', $ids)->update([
                'acknowledgment' => 1,
            ]);
    ...
    public function editRecord(Request $request)
    {
        ...
        $recordExists = \DB::table('udip')->where('id', $recordId)->exists();

        if (!$recordExists) {
            return response()->json(['message' => 'Запись с указанным ID не найдена.'], 404);
        }

        \DB::table('udip')->where('id', $recordId)->update($data);
        return response()->json(['status' => true]);
