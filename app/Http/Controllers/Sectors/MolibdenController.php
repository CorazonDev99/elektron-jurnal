...
class MolibdenController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('lisok.molibden.index', compact('userId', 'roleId'));
    }
    ...
    public function deleteRecord(Request $request)
    {
        ...
        try {
            ...
            \DB::table('molibden')->whereIn('id', $ids)->delete();
    ...
    public function getData(Request $request)
    {
        ...
        $d->selectRaw("'' as `empty`, molibden.id,molibden.user_id, molibden.role_id, DATE_FORMAT(molibden.created_at, '%d-%m-%Y %H:%i') AS created_at,
                DATE_FORMAT(molibden.updated_at, '%d-%m-%Y %H:%i') AS updated_at, molibden.equipment, molibden.problem, molibden.solution,
                molibden.responsible_person, DATE_FORMAT(molibden.deadline, '%d-%m-%Y') AS deadline, molibden.acknowledgment, molibden.resolved, molibden.reason, molibden.employee_name");
        if ($request->filled('fio')) {
            $d->where('molibden.employee_name', 'like', '%' . $request->fio . '%');
        }
        if ($request->filled('created_at')) {
            $d->whereDate('molibden.created_at', '=', $request->created_at);
        }
        if ($request->filled('equipment')) {
            $d->where('molibden.equipment', 'like', '%' . $request->equipment . '%');
        }
        if ($request->filled('problem')) {
            $d->where('molibden.problem', 'like', '%' . $request->problem . '%');
        }
        if ($request->filled('solution')) {
            $d->where('molibden.solution', 'like', '%' . $request->solution . '%');
        }
        if ($request->filled('responsible_person')) {
            $d->where('molibden.responsible_person', 'like', '%' . $request->responsible_person . '%');
        }
        if ($request->filled('deadline_from') && $request->filled('deadline_until')) {
            $d->whereBetween('molibden.deadline', [$request->deadline_from, $request->deadline_until])
                ->orWhereBetween('molibden.deadline', [$request->deadline_from, $request->deadline_until]);
        }

        elseif ($request->filled('deadline_from')) {
            $d->where('molibden.deadline', '>=', $request->deadline_from)
                ->orWhere('molibden.deadline', '>=', $request->deadline_until);
        }
        if ($request->filled('resolved')) {
            $d->where('molibden.resolved', 'like', '%' . $request->resolved . '%');
        }
        if ($request->filled('acknowledgment')) {
            $d->where('molibden.acknowledgment', $request->acknowledgment);
        }
    ...
    public function updateRecord(Request $request)
    {
        ...
        try {
            ...
            \DB::table('molibden')->whereIn('id', $ids)->update([
                'acknowledgment' => 1,
            ]);
    ...
    public function editRecord(Request $request)
    {
        ...
        $recordExists = \DB::table('molibden')->where('id', $recordId)->exists();

        if (!$recordExists) {
            return response()->json(['message' => 'Запись с указанным ID не найдена.'], 404);
        }

        \DB::table('molibden')->where('id', $recordId)->update($data);
        return response()->json(['status' => true]);
