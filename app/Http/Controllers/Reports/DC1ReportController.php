...
class DC1ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.dc1.index', compact('roleId', 'months', 'completed', 'inProgress', 'notCompleted', 'total', 'viewMode'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = DB::table('dc1')
            ->select(
                ...
                'dc1.solution',
                'dc1.resolved',
                'dc1.responsible_person',
                'dc1.deadline',
                DB::raw("DATE_FORMAT(dc1.created_at, '%d-%m-%Y %H:%i') AS created_at"),
                DB::raw("DATE_FORMAT(dc1.updated_at, '%d-%m-%Y %H:%i') AS updated_at"),
            ...
            ->whereMonth('dc1.created_at', '=', date('n', strtotime($month)))
            ->whereYear('dc1.created_at', '=', now()->year)
            ->groupBy('dc1.id', 'dc1.employee_name', 'dc1.equipment', 'dc1.problem', 'dc1.solution', 'dc1.responsible_person', 'dc1.deadline', 'dc1.created_at', 'dc1.updated_at');
