...
class Sector2ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.sector2.index', compact('roleId', 'months', 'completed', 'inProgress', 'notCompleted', 'total', 'viewMode'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = DB::table('sector2')
            ->select(
                'sector2.id',
                'sector2.employee_name',
                'sector2.equipment',
                'sector2.problem',
                'sector2.solution',
                'sector2.resolved',
                'sector2.responsible_person',
                'sector2.deadline',
                DB::raw("DATE_FORMAT(sector2.created_at, '%d-%m-%Y %H:%i') AS created_at"),
                DB::raw("DATE_FORMAT(sector2.updated_at, '%d-%m-%Y %H:%i') AS updated_at"),
            ...
            ->whereMonth('sector2.created_at', '=', date('n', strtotime($month)))
            ->whereYear('sector2.created_at', '=', now()->year)
            ->groupBy('sector2.id', 'sector2.employee_name', 'sector2.equipment', 'sector2.problem', 'sector2.solution', 'sector2.responsible_person', 'sector2.deadline', 'sector2.created_at', 'sector2.updated_at');
