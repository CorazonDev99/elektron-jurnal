...
class ReagentReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.reagent.index', compact('roleId', 'months', 'completed', 'inProgress', 'notCompleted', 'total', 'viewMode'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = DB::table('reagent')
            ->select(
                'reagent.id',
                'reagent.employee_name',
                'reagent.equipment',
                'reagent.problem',
                'reagent.solution',
                'reagent.resolved',
                'reagent.responsible_person',
                'reagent.deadline',
                DB::raw("DATE_FORMAT(reagent.created_at, '%d-%m-%Y %H:%i') AS created_at"),
                DB::raw("DATE_FORMAT(reagent.updated_at, '%d-%m-%Y %H:%i') AS updated_at"),
            ...
            ->whereMonth('reagent.created_at', '=', date('n', strtotime($month)))
            ->whereYear('reagent.created_at', '=', now()->year)
            ->groupBy('reagent.id', 'reagent.employee_name', 'reagent.equipment', 'reagent.problem', 'reagent.solution', 'reagent.responsible_person', 'reagent.deadline', 'reagent.created_at', 'reagent.updated_at');
