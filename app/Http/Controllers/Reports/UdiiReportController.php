...
class UdiiReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.udii.index', compact('roleId', 'months', 'completed', 'inProgress', 'notCompleted', 'total', 'viewMode'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = DB::table('udii')
            ->select(
                ...
                'udii.solution',
                'udii.resolved',
                'udii.responsible_person',
                'udii.deadline',
                DB::raw("DATE_FORMAT(udii.created_at, '%d-%m-%Y %H:%i') AS created_at"),
                DB::raw("DATE_FORMAT(udii.updated_at, '%d-%m-%Y %H:%i') AS updated_at"),
            ...
            ->whereMonth('udii.created_at', '=', date('n', strtotime($month)))
            ->whereYear('udii.created_at', '=', now()->year)
            ->groupBy('udii.id', 'udii.employee_name', 'udii.equipment', 'udii.problem', 'udii.solution', 'udii.responsible_person', 'udii.deadline', 'udii.created_at', 'udii.updated_at');
