...
class UdipReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.udip.index', compact('roleId', 'months', 'completed', 'inProgress', 'notCompleted', 'total', 'viewMode'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = DB::table('udip')
            ->select(
                ...
                'udip.solution',
                'udip.resolved',
                'udip.responsible_person',
                'udip.deadline',
                DB::raw("DATE_FORMAT(udip.created_at, '%d-%m-%Y %H:%i') AS created_at"),
                DB::raw("DATE_FORMAT(udip.updated_at, '%d-%m-%Y %H:%i') AS updated_at"),
            ...
            ->whereMonth('udip.created_at', '=', date('n', strtotime($month)))
            ->whereYear('udip.created_at', '=', now()->year)
            ->groupBy('udip.id', 'udip.employee_name', 'udip.equipment', 'udip.problem', 'udip.solution', 'udip.responsible_person', 'udip.deadline', 'udip.created_at', 'udip.updated_at');
