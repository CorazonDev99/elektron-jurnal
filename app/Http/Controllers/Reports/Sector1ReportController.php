...
class Sector1ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.sector1.index', compact('roleId', 'months', 'completed', 'inProgress', 'notCompleted', 'total', 'viewMode'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = DB::table('sector1')
            ->select(
                'sector1.id',
                'sector1.employee_name',
                'sector1.equipment',
                'sector1.problem',
                'sector1.solution',
                'sector1.resolved',
                'sector1.responsible_person',
                'sector1.deadline',
                DB::raw("DATE_FORMAT(sector1.created_at, '%d-%m-%Y %H:%i') AS created_at"),
                DB::raw("DATE_FORMAT(sector1.updated_at, '%d-%m-%Y %H:%i') AS updated_at"),
            ...
            ->whereMonth('sector1.created_at', '=', date('n', strtotime($month)))
            ->whereYear('sector1.created_at', '=', now()->year)
            ->groupBy('sector1.id', 'sector1.employee_name', 'sector1.equipment', 'sector1.problem', 'sector1.solution', 'sector1.responsible_person', 'sector1.deadline', 'sector1.created_at', 'sector1.updated_at');
